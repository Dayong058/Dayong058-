...
const HotelAdmin = (() => {
  ...
  function setPanel(tab) {
    ...
    const desc = qs("pageDesc");
    if (title) title.textContent = titleMap[tab]?.[0] || "后台管理";
    if (desc) desc.textContent = titleMap[tab]?.[1] || "";
  }
  ...
  function resetRoomForm() {
    ...
    if (qs("uploadBar")) qs("uploadBar").style.width = "0%";
    if (qs("uploadMsg")) qs("uploadMsg").textContent = "";
  }
        ...
        <td>
          ...
          <div class="muted" style="font-size:12px;margin-top:4px">
            ${
              r.cover
                ? `
                  <a href="${escapeHtml(r.cover)}" target="_blank">
                    <img
                      src="${escapeHtml(r.cover)}"
                      alt="room-cover"
                      loading="lazy"
                      style="
                        width: 120px;
  ...
  function resetBannerForm() {
    ...
    if (qs("banUploadBar")) qs("banUploadBar").style.width = "0%";
    if (qs("banUploadMsg")) qs("banUploadMsg").textContent = "";
    if (qs("banUploadFile")) qs("banUploadFile").value = "";
  }
      ...
      tr.innerHTML = `
        ...
        <td>
          ${
            b.image
              ? `
                <a href="${escapeHtml(b.image)}" target="_blank">
                  <img
  ...
  function uploadBannerImage() {
    const fileInput = qs("banUploadFile");
    const msgEl = qs("banUploadMsg");
    const barEl = qs("banUploadBar");
    const imageInput = qs("banImage");
    ...
    xhr.upload.onprogress = (evt) => {
      ...
      if (msgEl) msgEl.textContent = `上传中：${pct}%`;
    };
    ...
    xhr.onload = () => {
      try {
        ...
        if (xhr.status >= 200 && xhr.status < 300 && data.ok && data.url) {
          ...
          if (msgEl) msgEl.textContent = `上传成功：${data.url}`;
          if (barEl) barEl.style.width = "100%";
          toast("上传成功，已自动填入轮播图片URL");
        } else {
          if (msgEl)
            msgEl.textContent = `上传失败：${data.error || xhr.status}`;
          toast("上传失败");
        }
      } catch {
        if (msgEl) msgEl.textContent = "上传失败：响应解析错误";
        toast("上传失败");
      }
    };

    xhr.onerror = () => {
      if (msgEl) msgEl.textContent = "上传失败：网络错误";
      toast("上传失败");
    };
  ...
  return {
    ...
    // orders
    //（如果你 HTML 里有“订单刷新按钮”，可暴露 loadOrders / renderOrders 也行）
  };
})();
